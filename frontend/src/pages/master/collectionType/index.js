...
export default function CollectionType ({ page }) {
  const { basic } = useAppContext();
  const { search, move, setMove, route } = basic;
  const location = useRouter();
  const { res, isLoading, isError } = useFetcher("collectiontype", page);
...
export async function getServerSideProps(ctx) {
  ...
  const baseUrl = `${process.env.API_URL}/api/collectiontype`;
